...
public class PlayerManager : MonoBehaviour
{
    ...
    public IEnumerator SetInterTimeSeedRoutine()
    {
        ...
        // For example: 2022-02-21T15:00:26.947Z
        Debug.Log(originalDate);

        // We want to have a new word each day, so we do not need the time
        // so let's first remove that from the string
        // Get the position of T
        int indexOfT = originalDate.IndexOf("T");
        dateSeed = originalDate.Substring(0, indexOfT);

        // Now we have the following:
        // 2022-02-21

        // Now we will remove the dashes from the string
        dateSeed = dateSeed.Replace("-", string.Empty);
    ...
    public void SubmitScore(int score)
    {
        LootLockerSDKManager.GetMemberRank(leaderboardID.ToString(), playerID, (response) =>
        {
            if (response.statusCode == 200)
            {
                ...
                string scoreSeed = seed.ToString();
                string metadataSeed = "";
    ...
    public void AllowedToPlayCheck()
    {
        ...
        LootLockerSDKManager.GetMemberRank(leaderboardID.ToString(), playerID, (response) =>
        {
            if (response.statusCode == 200)
            {
                ...
                string todaysSeed = seed.ToString();
                string metadataSeed = "";
                if (response.metadata != null)
                {
                    metadataSeed = response.metadata.ToString();
    ...
    public IEnumerator SetupFriends()
    {
        ...
        Debug.Log("Amount of friends: " + tempFriendNames.Count);
